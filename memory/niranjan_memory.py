...
def speak(audio):
    """इस फंक्शन से Niranjan बोलता है और बातचीत को मेमोरी में सेव करता है"""
    print(f"Niranjan: {audio}")
    if engine:
        engine.say(audio)
        engine.runAndWait()
    append_conversation_sync("niranjan", audio)
...
async def get_recent_conversations(limit: int = 10) -> str:
    ...
    try:
        ...
        for entry in recent:
            speaker = "आप" if entry.get("speaker") == "user" else "निरंजन"
            text = entry.get("text", "")
