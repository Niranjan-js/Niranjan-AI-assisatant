# nova_full_niranjan.py
# NIRANJAN HUD — v3.0 (Master Tech Edition)
# Requirements: PySide6 (required), psutil (optional), opencv-python (optional)
# Run: pip install PySide6 psutil opencv-python
# Save as nova_full_niranjan.py and run: python nova_full_niranjan.py
...
class EnergyCore(QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.setFont(QFont(font_name,28,QFont.Black)); p.drawText(rect,Qt.AlignCenter,"NIRANJAN")
...
class NovaHUD(QMainWindow):
    def __init__(self):
        ...
        self.append_console("[NIRANJAN] Systems online. Welcome back, sir.")
    ...
    def tick(self):
        ...
        if random.random() < 0.05:
            self.append_console("[NIRANJAN] " + random.choice(["Monitoring systems.","All subsystems nominal."]))
    ...
    def open_settings(self):
        ...
        if dlg.exec(): self.apply_settings(dlg.get_values()); self.append_console("[NIRANJAN] Settings applied.")

    def cycle_theme(self):
        self.theme_index = (self.theme_index + 1) % len(THEME_ORDER); self.theme_key = THEME_ORDER[self.theme_index]
        self.energy.set_theme(self.theme_key); self.bg_widget.set_theme(self.theme_key); self.append_console(f"[NIRANJAN] Theme set to {self.theme_key.upper()}.")

    def toggle_console(self):
        self.console.setVisible(not self.console.isVisible()); self.append_console(f"[NIRANJAN] Console {'visible' if self.console.isVisible() else 'hidden'}.")

    def fake_voice(self):
        self.append_console("[NIRANJAN] Voice mode activated — listening..."); self.energy.set_speed(self.settings['speed']*2.6); self.energy.set_glow(2.4)
        QTimer.singleShot(2200, lambda: (self.energy.set_speed(self.settings['speed']), self.energy.set_glow(self.settings['glow']), self.append_console("[NIRANJAN] Voice mode idle.")))

    def toggle_camera(self):
        self.camera_running = getattr(self.camera.capture, 'isOpened', lambda: False)()
        if not self.camera_running:
            if self.camera.start_camera(): self.btn_cam_toggle.setText("Stop Camera"); self.append_console("[NIRANJAN] Camera started.")
            else: QMessageBox.warning(self, "Camera", "Unable to start camera."); self.append_console("[NIRANJAN] Camera start failed.")
        else: self.camera.stop_camera(); self.btn_cam_toggle.setText("Start Camera"); self.append_console("[NIRANJAN] Camera stopped.")
    ...
    def camera_snapshot(self):
        ...
        if path and self.camera.snapshot(path): self.append_console(f"[NIRANJAN] Snapshot saved."); QMessageBox.information(self, "Snapshot", "Saved.")
        elif path: self.append_console("[NIRANJAN] Snapshot failed."); QMessageBox.warning(self, "Snapshot", "Failed to save.")

    def save_screenshot(self):
        path, _ = QFileDialog.getSaveFileName(self, "Save HUD Screenshot", f"hud_{safe_now_str()}.png", "PNG Files (*.png)")
        if path:
            try: self.grab().save(path); self.append_console(f"[NIRANJAN] Screenshot saved."); QMessageBox.information(self, "Screenshot", "Saved.")
            except Exception as e: self.append_console(f"[NIRANJAN] Screenshot failed: {e}"); QMessageBox.warning(self, "Screenshot", "Failed to save.")
    ...
    def open_screen_share(self):
        ...
        try:
            import subprocess
            import sys
            script_path = os.path.join(os.path.dirname(__file__), "niranjan_gui_screen_share.py")
            if os.path.exists(script_path):
                subprocess.Popen([sys.executable, script_path])
                self.append_console("[NIRANJAN] Screen Vision window opened.")
            else:
                QMessageBox.warning(self, "Screen Vision", "niranjan_gui_screen_share.py not found!")
                self.append_console("[NIRANJAN] Screen Vision file not found.")
    ...
    def closeEvent(self, event):
        self.append_console("[NIRANJAN] Shutting down...")
